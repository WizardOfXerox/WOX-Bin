...
export async function DELETE(request: Request) {
  ...
  if (!row.passwordHash) {
    return jsonError("Create a password before disconnecting Google.", 409);
  }
  ...
  if (!googleAccount) {
    return jsonError("Google is not connected to this account.", 404);
  }
  ...
  await db.transaction(async (tx) => {
    await tx.delete(accounts).where(and(eq(accounts.userId, session.user.id), eq(accounts.provider, "google")));
  ...
  await logAudit({
    ...
    action: "settings.account.google_disconnected",
    targetType: "user",
