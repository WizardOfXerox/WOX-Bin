  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-6 px-4 py-8 md:px-6 md:py-10">
      ...
      <nav className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted-foreground">
        ...
        <span className="font-medium text-foreground">{t("nav.archive")}</span>
        <span aria-hidden className="text-border">
          /
        </span>
      ...
      {pastes.length === 0 ? (
        ...
        <div className="glass-panel overflow-hidden">
            ...
            {pastes.map((paste) => (
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4" key={paste.id}>
                <Link
                  ...
                  href={`/p/${paste.slug}`}
                >
                  {paste.title?.trim() || copy.untitled}
                </Link>
            ...
            <table className="w-full min-w-[520px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-white/10 bg-white/[0.04] text-xs uppercase tracking-wider text-muted-foreground">
                  <th className="px-4 py-3 font-medium md:px-5">{copy.nameTitle}</th>
                  <th className="whitespace-nowrap px-4 py-3 font-medium md:px-5">{copy.posted}</th>
                  <th className="whitespace-nowrap px-4 py-3 font-medium md:px-5">{copy.syntax}</th>
                  <th className="whitespace-nowrap px-4 py-3 font-medium md:px-5">{copy.views}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="border-t border-white/10 px-4 py-3 text-xs text-muted-foreground md:px-5">
            {copy.showing
              .replace("{count}", String(pastes.length))
              .replace("{suffix}", pastes.length === 1 ? "" : "s")
              .replace("{sPlural}", pastes.length === 1 ? "" : "s")}{" "}
