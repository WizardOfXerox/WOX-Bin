...
function normalizePaste(paste: Partial<PasteDraft> & Pick<PasteDraft, "title" | "content">): PasteDraft {
  ...
  return {
    ...
    viewCount: Number.isFinite(paste.viewCount) ? Math.max(0, Number(paste.viewCount)) : 0,
...
export function createEmptyPaste(overrides?: Partial<PasteDraft>): PasteDraft {
  ...
  return normalizePaste({
    ...
    viewCount: Number.isFinite(overrides?.viewCount) ? Math.max(0, Number(overrides?.viewCount)) : 0,
    ...
    secretMode: Boolean(overrides?.secretMode),
    captchaRequired: Boolean(overrides?.captchaRequired),
